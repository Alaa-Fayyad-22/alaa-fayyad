...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { name, email, projectType, budget, timeline, details } = req.body;
       ...
       html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; background: #f8f8fc; border-radius: 12px;">
          ...
          <div style="background: white; padding: 24px; border-radius: 10px; border: 1px solid #e5e7eb;">
            ...
            <p style="margin: 0 0 12px;"><strong style="color: #6366f1;">Project Type:</strong> ${projectType || 'Not specified'}</p>
            <p style="margin: 0 0 12px;"><strong style="color: #6366f1;">Budget:</strong> ${budget || 'Not specified'}</p>
            <p style="margin: 0 0 12px;"><strong style="color: #6366f1;">Timeline:</strong> ${timeline || 'Not specified'}</p>
            <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 16px 0;" />
            <p style="margin: 0 0 16px; line-height: 1.7; color: #374151;">
              <strong style="color: #6366f1;">Project Details:</strong><br/>${details.replace(/\n/g, '<br/>')}
            </p>
            <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 16px 0;" />
